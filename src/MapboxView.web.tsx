...
function InternalMapBoxMapView({
  ...
  const typedControllerRef = useRef<MapboxViewController | null>(null);
  ...
  useEffect(() => {
    ...
    provider
      ...
      .then((ctrl) => {
        ...
        setController(ctrl);
        typedControllerRef.current = ctrl as MapboxViewController;
      ...
      .catch((error) => {
        ...
        if (MapboxProvider.isDestroyedBeforeLoad(error)) return;
...
export function MapBoxMapView(props: MapBoxMapViewProps) {
  return <InternalMapBoxMapView {...props} projection={props.projection ?? MapProjection.Globe} />;
}

export function MapBoxMapView2D(props: MapBoxMapViewProps) {
  return <InternalMapBoxMapView {...props} projection={props.projection ?? MapProjection.Mercator} />;
}
