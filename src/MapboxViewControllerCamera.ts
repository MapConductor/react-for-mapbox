...
import type { MapboxMapViewHolder } from './MapboxMapViewHolder';
 ...
 */
export function jumpToPosition(map: MapboxMap, position: MapCameraPosition): Promise<boolean> {
...
export function easeToPosition(map: MapboxMap, position: MapCameraPosition, durationMillis: number): Promise<boolean> {
  const cam = toCameraPosition(position);
  return new Promise((resolve) => {
    map.once('moveend', () => resolve(true));
    map.easeTo({
...
export function fitMapBounds(map: MapboxMap, bounds: GeoRectBounds, padding: number): Promise<boolean> {
  return new Promise((resolve) => {
    map.once('moveend', () => resolve(true));
 ...
 * `MapboxViewControllerImpl.getMapCameraPosition()`.
 */
export function readVisibleRegion(map: MapboxMap, holder: MapboxMapViewHolder): VisibleRegion | null {
