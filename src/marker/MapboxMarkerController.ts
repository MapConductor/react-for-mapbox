...
import { MapboxMarkerOverlayRenderer } from './MapboxMarkerOverlayRenderer';
import {
...
let mapboxTileProtocolRegistered = false;
...
function registerMapboxTileProtocol(): void {
  if (mapboxTileProtocolRegistered) return;

  const protocolApi = mapboxgl as unknown as {
    addProtocol?: (
  ...
  mapboxTileProtocolRegistered = true;
}
...
export class MapboxMarkerController extends AbstractMarkerController<MapboxActualMarker> {
  declare readonly renderer: MapboxMarkerOverlayRenderer;

  private selected: MarkerEntity<MapboxActualMarker> | null = null;
  ...
  override find(position: GeoPoint): MarkerEntity<MapboxActualMarker> | null {
  ...
  ): MarkerEntity<MapboxActualMarker> | null {
    // Defers to Mapbox's own rendered-geometry query (`queryRenderedFeatures`),
  ...
  async setSelectedMarker(
    entity: MarkerEntity<MapboxActualMarker> | null,
  ): Promise<void> {
    if (!entity) {
      const selected = this.selected;
      if (!selected) return;
