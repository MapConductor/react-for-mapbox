...
export class MapboxMarkerOverlayRenderer extends AbstractMarkerOverlayRenderer<
  MapboxMapViewHolder,
  MapboxActualMarker
> {
  ...
  async onAdd(data: AddParams[]): Promise<(MapboxActualMarker | null)[]> {
  ...
  async onChange(
    data: ChangeParams<MapboxActualMarker>[],
  ): Promise<(MapboxActualMarker | null)[]> {
  ...
  async onRemove(data: MarkerEntity<MapboxActualMarker>[]): Promise<void> {
  ...
  override setMarkerVisible(entity: MarkerEntity<MapboxActualMarker>, visible: boolean): void {
  ...
  setMarkerPosition(
    ...
    entity.marker = {
      ...
      properties: {
        ...
        [MapboxMarkerProp.Z_INDEX]: resolveZIndex(entity.state),
      },
    };
  ...
  private async createMarkerFeature(
    ...
    return {
      ...
      properties: {
        [MapboxMarkerProp.ID]: state.id,
        [MapboxMarkerProp.ICON_ID]: iconId,
        [MapboxMarkerProp.Z_INDEX]: resolveZIndex(state),
      },
    };
  }
  ...
  private async loadBitmapIcon(
    ...
    try {
      return await new Promise<HTMLImageElement | ImageBitmap | ImageData>((resolve, reject) => {
        this.holder.map.loadImage(url, (error, image) => {
          if (error || !image) {
            reject(error ?? new Error(`Failed to load marker icon: ${url}`));
            return;
          }
          resolve(image);
        });
  ...
  buildEntity(
    marker: MapboxActualMarker,
    state: MarkerState,
  ): MarkerEntity<MapboxActualMarker> {
