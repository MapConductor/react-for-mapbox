...
export type MapboxActualMarker = PointFeature;

export const MapboxMarkerProp = {
  ID: 'mc-id',
  ICON_ID: 'mc-icon-id',
  Z_INDEX: 'mc-z-index',
} as const;
...
export class MarkerLayer {
    ...
    try {
      ...
      if (!map.getLayer(this.layerId)) {
        map.addLayer({
          ...
          layout: {
            'icon-image': ['get', MapboxMarkerProp.ICON_ID],
            'icon-allow-overlap': true,
            'icon-ignore-placement': true,
            'symbol-sort-key': ['get', MapboxMarkerProp.Z_INDEX],
  ...
  setIconOffsets(
    ...
    const expression: unknown[] = ['match', ['get', MapboxMarkerProp.ICON_ID]];
